...
| 项 | 内容 | 状态 |
|----|------|------|
| P2-1 | `.env` 配置 JWT / 微信 Secret / 模板 ID | ✅ 模板 `.env.production.example` |
| P2-2 | 阿里云 OCR + 内容安全正式开通 | 🔲 需运维开通 |
| P2-3 | DeepSeek 客服（可选） | 🔲 可选 |
| P2-4 | 服务端减少 `api/mock.js` 热路径依赖 | 🔲 |
| P2-5 | `pnpm sync:prod` + HTTPS 域名 | ✅ 脚本 + 文档 |
...
| P7-3 | GitHub Actions 与 remote 联动 | ✅ verify + publish + SECRETS.md |
...
```bash
pnpm ci:verify              # 静态 CI（11 项，含密钥模板）
pnpm verify:env             # 生产 .env 模板完整性
